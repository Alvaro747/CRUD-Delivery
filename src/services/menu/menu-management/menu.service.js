class MenuService {
  ...
  async create (data) {
    try {
      if (!data || !data.name) {
        return this._utilities.io.response.error('Please provide name')
      }

      data.id = this._utilities.generator.id({ length: 15, prefix: 'menu-' })

      const entity = new this._models.MenuManagementModel(data, this._dependencies)
  ...
  async delete (data) {
    data.status = this.status.deleted // assign the deleted property to the menu
    try {
      if (!data || !data.id) {
        return this._utilities.io.response.error('Please provide an id')
      }
  ...
  get status () {
    return this._models.MenuManagementModel.statuses
  }
}
